  ...
  <div class="container">
    ...
    <section class="row pb-4 pb-md-5 mb-2 mb-md-0 mb-xl-3">
      ...
      <div class="col-md-7 col-xl-8 pb-4 pb-md-0 mb-2 mb-sm-3 mb-md-0">
        <div class="row row-cols-2 g-3 g-sm-4 g-md-3 g-lg-4">
          @if($attachments->count() > 0)
              ...
              @if($index < 2)
                <div class="col">
                  <a class="hover-effect-scale hover-effect-opacity position-relative d-flex rounded-4 overflow-hidden"
                    href="{{ asset('storage/' . $attachment->filePath) }}" data-glightbox="" data-gallery="product-gallery">
                    <i
                    ...
                    <div class="ratio ratio-1x1 hover-effect-target">
                      <img src="{{ asset('storage/' . $attachment->filePath) }}" alt="{{ $product->productName }}">
                    </div>
                  </a>
                </div>
              @endif
            @endforeach
          @else
            <!-- Default images if no attachments -->
            <div class="col">
              <a class="hover-effect-scale hover-effect-opacity position-relative d-flex rounded-4 overflow-hidden"
                href="{{ asset('frontend/assets/img/shop/furniture/product/01.png') }}" data-glightbox=""
                data-gallery="product-gallery">
                <i
                  class="ci-zoom-in hover-effect-target fs-3 text-white position-absolute top-50 start-50 translate-middle opacity-0 z-2"></i>
                <div class="ratio ratio-1x1 hover-effect-target">
                  <img src="{{ asset($product->thumbnail) }}" alt="{{ $product->productName }}">
                </div>
              </a>
            </div>
                ...
                <div class="row row-cols-2 g-3 g-sm-4 g-md-3 g-lg-4 pb-3 pb-sm-4 pb-md-0">
                  @foreach($attachments as $index => $attachment)
                    @if($index >= 2)
                      <div class="col">
                        <a class="hover-effect-scale hover-effect-opacity position-relative d-flex rounded-4 overflow-hidden"
                          href="{{ asset('storage/' . $attachment->filePath) }}" data-glightbox=""
                          data-gallery="product-gallery">
                          <span
      ...
      <div class="col-md-5 col-xl-4">
        ...
        <div class="sticky-md-top ps-md-2 ps-xl-4">
          ...
          <h1 class="fs-xl fw-medium">{{ $product->productName }}</h1>
          <div class="h4 fw-bold mb-4">
            ...
            @if($product->originalPrice && $product->originalPrice > $product->price)
              <del class="fs-sm fw-normal text-body-tertiary">${{ number_format($product->originalPrice, 2) }}</del>
            @endif
          </div>
          <ul class="list-unstyled fs-sm text-body-emphasis mb-4">
            <li>
              <span class="me-1">Pay 4 interest-free payments of <span
                  class="fw-semibold">${{ number_format($product->price / 4, 2) }}</span> with</span>
          ...
          @if($product->colors && count($product->colors) > 0)
            <div class="mb-4">
              <label class="form-label fw-semibold pb-1 mb-2">Color: <span class="text-body fw-normal"
                  id="colorOption">{{ $product->colors[0]->name ?? 'Default' }}</span></label>
              <div class="d-flex flex-wrap gap-2" data-binded-label="#colorOption">
                @foreach($product->colors as $index => $color)
                  <input type="radio" class="btn-check" name="colors" id="color-{{ $color->id }}" {{ $index === 0 ? 'checked' : '' }}>
                  <label for="color-{{ $color->id }}" class="btn btn-image p-0" data-label="{{ $color->name }}">
                    <img src="{{ asset('storage/' . $color->image) }}" width="56" alt="{{ $color->name }} color">
        ...
        <div class="navbar flex-nowrap align-items-center bg-body pt-4 pb-2">
          <div class="d-flex align-items-center min-w-0 ms-lg-2 me-3">
            <div class="ratio ratio-1x1 flex-shrink-0" style="width: 50px">
              @if($attachments->count() > 0)
                <img src="{{ asset('storage/' . $attachments->first()->filePath) }}" alt="{{ $product->productName }}">
              @else
                <img src="{{ asset('frontend/assets/img/shop/furniture/product/thumb.png') }}"
                  alt="{{ $product->productName }}">
              @endif
            </div>
            <h4 class="h6 fw-medium d-none d-lg-block ps-3 mb-0">{{ $product->productName }}</h4>
            <div class="w-100 min-w-0 d-lg-none ps-2">
              <h4 class="fs-sm fw-medium text-truncate mb-1">{{ $product->productName }}</h4>
              <div class="h6 mb-0">
                ₹{{ number_format($product->price, 2) }}
                @if($product->originalPrice && $product->originalPrice > $product->price)
                  <del class="fs-xs fw-normal text-body-tertiary">${{ number_format($product->originalPrice, 2) }}</del>
                @endif
              </div>
            </div>
          </div>
          ...
          <div class="d-flex gap-2">
            ...
            <button type="button" class="btn btn-dark rounded-pill ms-auto d-none d-md-inline-flex px-4">Add to
              cart</button>
            <button type="button" class="btn btn-icon btn-dark rounded-circle animate-slide-end ms-auto d-md-none"
    ...
    <section class="row pb-5 mb-2 mb-sm-3 mb-lg-4 mb-xl-5">
      <div class="col-md-7 col-xl-8 mb-xxl-3">
        <p>
          {{ $product->shortDescription ?? 'The chair will bring a stylish retro atmosphere to your room, inspired by the Scandinavian design. The classic look fits anywhere in your home and provides a sturdy and durable place to sit for years to come.' }}
        </p>

        @if($product->specifications && count($product->specifications) > 0)
          <ul class="list-unstyled pb-md-2 pb-lg-3">
            @foreach($product->specifications as $spec)
              <li class="mt-1"><span class="h6 mb-0">{{ $spec->name }}:</span> {{ $spec->value }}</li>
            @endforeach
          </ul>
        @endif
        ...
        <div class="accordion accordion-alt-icon" id="productAccordion">
          <div class="accordion-item">
            <h3 class="accordion-header" id="headingProductInfo">
              <button type="button" class="accordion-button animate-underline fs-xl collapsed" data-bs-toggle="collapse"
                data-bs-target="#productInfo" aria-expanded="false" aria-controls="productInfo">
          ...
          @if($product->features && count($product->features) > 0)
            <div class="accordion-item">
              <h3 class="accordion-header" id="headingProductFeatures">
                <button type="button" class="accordion-button animate-underline fs-xl collapsed" data-bs-toggle="collapse"
                  data-bs-target="#productFeatures" aria-expanded="false" aria-controls="productFeatures">
                  <span class="animate-target me-2">Features</span>
                </button>
              </h3>
              <div class="accordion-collapse collapse" id="productFeatures" aria-labelledby="headingProductFeatures"
                data-bs-parent="#productAccordion">
                <div class="accordion-body fs-base">
                  <ul class="m-0">
                    @foreach($product->features as $feature)
                      <li><span class="text-body-emphasis fw-semibold">{{ $feature->name }}:</span>
          ...
          <div class="accordion-item">
            <h3 class="accordion-header" id="headingProductDelivery">
              <button type="button" class="accordion-button animate-underline fs-xl collapsed" data-bs-toggle="collapse"
                data-bs-target="#productDelivery" aria-expanded="false" aria-controls="productDelivery">
                <span class="animate-target me-2">Delivery and shipping</span>
              </button>
            </h3>
            <div class="accordion-collapse collapse" id="productDelivery" aria-labelledby="headingProductDelivery"
              data-bs-parent="#productAccordion">
              <div class="accordion-body fs-base">
                {{ $product->deliveryInfo ?? 'We understand the importance of timely delivery and strive to provide a seamless shipping experience for our customers. Upon placing your order, our team will process it promptly, and you will receive a notification once your chair is ready for shipment. We offer various shipping options to accommodate your preferences, with estimated delivery times provided at checkout. Rest assured, your chair will be carefully packaged to ensure it arrives safely at your doorstep, ready to enhance your home with its timeless charm.' }}
              </div>
            </div>
          </div>
        </div>
    ...
    <!-- Product gallery and details -->
    <section class="row pb-4 pb-md-5 mb-2 mb-md-0 mb-xl-3 d-none">
          ...
          <div class="col">
            <a class="hover-effect-scale hover-effect-opacity position-relative d-flex rounded-4 overflow-hidden"
              ...
              <i
                class="ci-zoom-in hover-effect-target fs-3 text-white position-absolute top-50 start-50 translate-middle opacity-0 z-2"></i>
              <div class="ratio ratio-1x1 hover-effect-target">
                <img src="{{ asset('frontend/assets/img/shop/furniture/product/01.png') }}" alt="Image">
              </div>
            </a>
          </div>
          <div class="col">
            <a class="hover-effect-scale hover-effect-opacity position-relative d-flex rounded-4 overflow-hidden"
              href="{{ asset('frontend/assets/img/shop/furniture/product/02.jpg')}}" data-glightbox=""
              data-gallery="product-gallery">
              <span
                class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-black bg-opacity-25 opacity-0 z-1"></span>
              <i
                class="ci-zoom-in hover-effect-target fs-3 text-white position-absolute top-50 start-50 translate-middle opacity-0 z-2"></i>
              <div class="ratio ratio-1x1 hover-effect-target">
                <img src="{{ asset('frontend/assets/img/shop/furniture/product/02.jpg')}}" alt="Image">
              </div>
            </a>
          </div>
          <div class="col-12">
            <div class="collapse d-md-block" id="morePictures">
              <div class="row row-cols-2 g-3 g-sm-4 g-md-3 g-lg-4 pb-3 pb-sm-4 pb-md-0">
                <div class="col">
                  <a class="hover-effect-scale hover-effect-opacity position-relative d-flex rounded-4 overflow-hidden"
                    href="{{ asset('frontend/assets/img/shop/furniture/product/03.jpg')}}" data-glightbox=""
                    data-gallery="product-gallery">
                    <span
                      class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-black bg-opacity-25 opacity-0 z-1"></span>
                    <i
                      class="ci-zoom-in hover-effect-target fs-3 text-white position-absolute top-50 start-50 translate-middle opacity-0 z-2"></i>
                    <div class="ratio ratio-1x1 hover-effect-target">
                      <img src="{{ asset('frontend/assets/img/shop/furniture/product/03.jpg')}}" alt="Image">
                    </div>
                  </a>
                </div>
                <div class="col">
                  <a class="hover-effect-scale hover-effect-opacity position-relative d-flex rounded-4 overflow-hidden"
                    href="{{ asset('frontend/assets/img/shop/furniture/product/04.jpg')}}" data-glightbox=""
                    data-gallery="product-gallery">
                    <span
                      class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-black bg-opacity-25 opacity-0 z-1"></span>
                    <i
                      class="ci-zoom-in hover-effect-target fs-3 text-white position-absolute top-50 start-50 translate-middle opacity-0 z-2"></i>
                    <div class="ratio ratio-1x1 hover-effect-target">
                      <img src="{{ asset('frontend/assets/img/shop/furniture/product/04.jpg')}}" alt="Image">
                    </div>
                  </a>
                </div>
                <div class="col">
                  <a class="hover-effect-scale hover-effect-opacity position-relative d-flex rounded-4 overflow-hidden"
                    href="{{ asset('frontend/assets/img/shop/furniture/product/05.jpg')}}" data-glightbox=""
                    data-gallery="product-gallery">
                    <span
                      class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-black bg-opacity-25 opacity-0 z-1"></span>
                    <i
                      class="ci-zoom-in hover-effect-target fs-3 text-white position-absolute top-50 start-50 translate-middle opacity-0 z-2"></i>
                    <div class="ratio ratio-1x1 hover-effect-target">
                      <img src="{{ asset('frontend/assets/img/shop/furniture/product/05.jpg')}}" alt="Image">
                    </div>
                  </a>
                </div>
                <div class="col">
                  <a class="hover-effect-scale hover-effect-opacity position-relative d-flex rounded-4 overflow-hidden"
                    href="{{ asset('frontend/assets/img/shop/furniture/product/06.jpg')}}" data-glightbox=""
                    data-gallery="product-gallery">
                    <span
                      class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-black bg-opacity-25 opacity-0 z-1"></span>
                    <i
                      class="ci-zoom-in hover-effect-target fs-3 text-white position-absolute top-50 start-50 translate-middle opacity-0 z-2"></i>
                    <div class="ratio ratio-1x1 hover-effect-target">
                      <img src="{{ asset('frontend/assets/img/shop/furniture/product/06.jpg')}}" alt="Image">
                    </div>
                  </a>
                </div>
              </div>
          ...
          <div class="mb-4">
            ...
            <div class="d-flex flex-wrap gap-2" data-binded-label="#colorOption">
              ...
              <label for="viridian" class="btn btn-image p-0" data-label="Viridian">
                <img src="{{ asset('frontend/assets/img/shop/furniture/product/colors/color01.png') }}" width="56"
                  alt="Viridian color">
              ...
              <label for="green" class="btn btn-image p-0" data-label="Green">
                <img src="{{ asset('frontend/assets/img/shop/furniture/product/colors/color02.png') }}" width="56"
                  alt="Green color">
                <span class="visually-hidden">Green</span>
              </label>
              <input type="radio" class="btn-check" name="colors" id="blue">
              <label for="blue" class="btn btn-image p-0" data-label="Blue">
                <img src="{{ asset('frontend/assets/img/shop/furniture/product/colors/color03.png') }}" width="56"
                  alt="Blue color">
    ...
    <!-- Sticky product preview + Add to cart CTA -->
    <section class="sticky-product-banner sticky-top d-none" data-sticky-element="">
      <div class="sticky-product-banner-inner pt-5">
        <div class="navbar flex-nowrap align-items-center bg-body pt-4 pb-2">
          <div class="d-flex align-items-center min-w-0 ms-lg-2 me-3">
            <div class="ratio ratio-1x1 flex-shrink-0" style="width: 50px">
              <img src="{{ asset('frontend/assets/img/shop/furniture/product/thumb.png') }}" alt="Image">
            </div>
    ...
    <!-- Product description -->
    <section class="row pb-5 mb-2 mb-sm-3 mb-lg-4 mb-xl-5 d-none">
      <div class="col-md-7 col-xl-8 mb-xxl-3">
        <p>The chair will bring a stylish retro atmosphere to your room, inspired by the Scandinavian design. The classic
          look fits anywhere in your home and provides a sturdy and durable place to sit for years to come.</p>
        <ul class="list-unstyled pb-md-2 pb-lg-3">
        ...
        <div class="accordion accordion-alt-icon" id="productAccordion">
          <div class="accordion-item">
            ...
            <div class="accordion-collapse collapse" id="productInfo" aria-labelledby="headingProductInfo"
              data-bs-parent="#productAccordion">
              <div class="accordion-body fs-base">Introducing our Scandinavian-inspired chair, meticulously designed to
                infuse your living space with a touch of retro elegance. Crafted with the finest materials and attention
                to
                detail, this chair embodies the timeless charm of Scandinavian design, making it a versatile addition to
                any
                home decor. With its classic silhouette and understated sophistication, it seamlessly integrates into
                various interior styles, bringing both style and functionality to your room.</div>
            </div>
          </div>
          <div class="accordion-item">
            <h3 class="accordion-header" id="headingProductFeatures">
            ...
            <div class="accordion-collapse collapse" id="productFeatures" aria-labelledby="headingProductFeatures"
              ...
              <div class="accordion-body fs-base">
                <ul class="m-0">
                  <li><span class="text-body-emphasis fw-semibold">Timeless design:</span> Inspired by Scandinavian
                    aesthetics, this chair boasts a sleek and retro-inspired silhouette that adds a touch of elegance to
                    any
                    space.</li>
                  <li><span class="text-body-emphasis fw-semibold">Durable construction:</span> Crafted with high-quality
                    materials, the chair is built to last, providing sturdy and reliable seating for years to come.</li>
                  <li><span class="text-body-emphasis fw-semibold">Versatile placement:</span> Whether as a focal point in
                    your living room, a cozy reading corner in your study, or an inviting seat around the dining table,
                    this
                    chair effortlessly adapts to different settings.</li>
                  <li><span class="text-body-emphasis fw-semibold">Comfortable seating:</span> The chair features a
                    well-padded seat and backrest, ensuring optimal comfort for extended periods of relaxation or
                    conversation.</li>
                  <li><span class="text-body-emphasis fw-semibold">Easy maintenance:</span> Designed for convenience, the
                    chair's upholstery is easy to clean, allowing for hassle-free maintenance and care.</li>
                </ul>
              </div>
            </div>
          </div>
          <div class="accordion-item">
            ...
            <div class="accordion-collapse collapse" id="productWarranty" aria-labelledby="headingProductWarranty"
              data-bs-parent="#productAccordion">
              <div class="accordion-body fs-base">We stand behind the quality of our products. Our chair comes with a
                10-year warranty, guaranteeing against defects in materials and workmanship under normal use. In the
                unlikely event that you encounter any issues with your chair, contact our customer service team, and we
                will
                be happy to assist you with a replacement or repair.</div>
            </div>
          </div>
          <div class="accordion-item">
            <h3 class="accordion-header" id="headingProductDelivery">
            ...
            <div class="accordion-collapse collapse" id="productDelivery" aria-labelledby="headingProductDelivery"
              data-bs-parent="#productAccordion">
              <div class="accordion-body fs-base">We understand the importance of timely delivery and strive to provide a
                seamless shipping experience for our customers. Upon placing your order, our team will process it
                promptly,
                and you will receive a notification once your chair is ready for shipment. We offer various shipping
                options
                to accommodate your preferences, with estimated delivery times provided at checkout. Rest assured, your
                chair will be carefully packaged to ensure it arrives safely at your doorstep, ready to enhance your home
                with its timeless charm.</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- Blog articles -->
    <section class="pb-5 mb-2 mb-sm-3 mb-lg-4 mb-xl-5 d-none">
      ...
      <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4 mb-xxl-3">
        ...
        <article class="col">
          <a class="ratio d-flex hover-effect-scale rounded-4 overflow-hidden" href="#!"
            ...
            <img src="{{ asset('frontend/assets/img/blog/grid/v2/10.jpg')}}" class="hover-effect-target" alt="Image">
          </a>
          <div class="pt-4">
            ...
            <h3 class="h6 mb-3">
              <a class="hover-effect-underline" href="#!">Furnishing your space: a guide to choosing the perfect furniture
                pieces</a>
            </h3>
        ...
        <article class="col">
          <a class="ratio d-flex hover-effect-scale rounded-4 overflow-hidden" href="#!"
            style="--cz-aspect-ratio: calc(260 / 306 * 100%)">
            <img src="{{ asset('frontend/assets/img/blog/grid/v2/11.jpg')}}" class="hover-effect-target" alt="Image">
          </a>
          <div class="pt-4">
            <div class="nav pb-2 mb-1">
            ...
            <h3 class="h6 mb-3">
              <a class="hover-effect-underline" href="#!">Transform your living space with these chic interior design
                tips</a>
            </h3>
            <div class="nav align-items-center gap-2 fs-xs">
        ...
        <article class="col">
          <a class="ratio d-flex hover-effect-scale rounded-4 overflow-hidden" href="#!"
            style="--cz-aspect-ratio: calc(260 / 306 * 100%)">
            <img src="{{ asset('frontend/assets/img/blog/grid/v2/13.jpg')}}" class="hover-effect-target" alt="Image">
          </a>
          <div class="pt-4">
            <div class="nav pb-2 mb-1">
    ...
    <section class="pb-5 mb-2 mb-sm-3 mb-lg-4 mb-xl-5 d-none">
        ...
        <div class="row py-sm-2">
          <div class="col-md-4 col-lg-3 mb-3 mb-md-0">
            <div class="d-flex h6 mb-2">
              Rafael Marquez
              <i class="ci-check-circle text-success mt-1 ms-2" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-sm"
          ...
          <div class="col-md-8 col-lg-9">
            <p class="mb-md-4">Absolutely love this chair! It's exactly what I was looking for to complete my
              Scandinavian-themed living room. The wooden legs add a touch of warmth and the design is timeless.
              Comfortable
              and sturdy, couldn't be happier with my purchase!</p>
      ...
      <div class="border-bottom py-4">
        <div class="row py-sm-2">
          ...
          <div class="col-md-8 col-lg-9">
            <p class="mb-md-4">While the design of the chair is nice and it does add a touch of retro vibe to my space, I
              found the quality to be lacking. After just a few weeks of use, one of the legs started to wobble, and the
              seat isn't as comfortable as I had hoped. Disappointed with the durability. Additionally, the assembly
              process
              was a bit frustrating as some of the screws didn't align properly with the holes, requiring extra effort to
              secure them in place. Overall, while it looks good, I expected better quality for the price.</p>
            <div class="d-sm-flex justify-content-between">
              <div class="d-flex align-items-center fs-sm fw-medium text-dark-emphasis pb-2 pb-sm-0 mb-1 mb-sm-0">
      ...
      <div class="border-bottom py-4">
        <div class="row py-sm-2">
          ...
          <div class="col-md-8 col-lg-9">
            <p class="mb-md-4">Great addition to our dining room! The chair looks fantastic and is quite comfortable for
              short sits. Assembly was a breeze, and the quality seems decent for the price. Overall, satisfied with the
              purchase.</p>
            <div class="d-sm-flex justify-content-between">
              <div class="d-flex align-items-center fs-sm fw-medium text-dark-emphasis pb-2 pb-sm-0 mb-1 mb-sm-0">
                <i class="ci-check fs-base me-1" style="margin-top: .125rem"></i>
      ...
      <div class="collapse" id="moreReviews">
        ...
        <div class="border-bottom py-4">
          <div class="row py-sm-2">
            ...
            <div class="col-md-8 col-lg-9">
              <p class="mb-md-4">Couldn't be happier with this chair! It's not only stylish but also incredibly
                comfortable.
                The size is perfect for our space, and the wooden legs feel sturdy. Definitely recommend it to anyone
                looking for a chic and functional seating option.</p>
              <div class="d-sm-flex justify-content-between">
                <div class="d-flex align-items-center fs-sm fw-medium text-dark-emphasis pb-2 pb-sm-0 mb-1 mb-sm-0">
                  <i class="ci-check fs-base me-1" style="margin-top: .125rem"></i>
                  Yes, I recommend this product
        ...
        <div class="border-bottom py-4">
          <div class="row py-sm-2">
            ...
            <div class="col-md-8 col-lg-9">
              <p class="mb-md-4">This chair exceeded my expectations! It's well-made, comfortable, and looks even better
                in
                person. The Scandinavian design adds a sophisticated touch to my home office. I've received so many
                compliments already. Five stars all the way!</p>
              <div class="d-sm-flex justify-content-between">
                <div class="d-flex align-items-center fs-sm fw-medium text-dark-emphasis pb-2 pb-sm-0 mb-1 mb-sm-0">
                  <i class="ci-check fs-base me-1" style="margin-top: .125rem"></i>
                  Yes, I recommend this product
        ...
        <div class="border-bottom py-4">
          <div class="row py-sm-2">
            ...
            <div class="col-md-8 col-lg-9">
              <p class="mb-md-4">The chair is nice, but it's not the most comfortable for extended periods of sitting. The
                wooden legs give it a nice aesthetic, but they seem a bit fragile. It's a decent chair for occasional use,
                but I wouldn't recommend it for daily use or long sitting sessions.</p>
              <div class="d-sm-flex justify-content-between">
                <div class="d-flex align-items-center fs-sm fw-medium text-dark-emphasis pb-2 pb-sm-0 mb-1 mb-sm-0">
                  <i class="ci-close fs-base me-1" style="margin-top: .125rem"></i>
                  No, I don't recommend this product
    ...
    <section class="pb-5 mb-2 mb-sm-3 mb-md-4 mb-lg-5 d-none">
        ...
        <div class="swiper" data-swiper="{
          ...
          <div class="swiper-wrapper">
            ...
            <div class="swiper-slide">
              <div class="animate-underline">
                <a class="hover-effect-opacity ratio ratio-1x1 d-block mb-3" href="#">
                  <img src="{{ asset('frontend/assets/img/shop/furniture/16.png') }}"
            ...
            <div class="swiper-slide">
              <div class="animate-underline">
                <a class="hover-effect-opacity ratio ratio-1x1 d-block mb-3" href="#">
                  <img src="{{ asset('frontend/assets/img/shop/furniture/10.png') }}"
                    class="hover-effect-target opacity-100" alt="Product">
            ...
            <div class="swiper-slide">
              <div class="animate-underline">
                <a class="hover-effect-opacity ratio ratio-1x1 d-block mb-3" href="#">
                  <img src="{{ asset('frontend/assets/img/shop/furniture/11.png') }}"
                    class="hover-effect-target opacity-100" alt="Product">
            ...
            <div class="swiper-slide">
              <div class="animate-underline">
                <a class="hover-effect-opacity ratio ratio-1x1 d-block mb-3" href="#">
                  <img src="{{ asset('frontend/assets/img/shop/furniture/03.png') }}"
                    class="hover-effect-target opacity-100" alt="Product">
