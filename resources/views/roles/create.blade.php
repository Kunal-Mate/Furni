        ...
        <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4">
            <div class="card-body px-4 py-3">
                <div class="row align-items-center">
                    <div class="col-9">
                        <h4 class="fw-semibold mb-0">Add Role</h4>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="card-body p-3">
                    <form method="post" enctype="multipart/form-data" action="{{ route('roles.store') }}">
                        ...
                        <div class="mb-3 col-4">
                            ...
                            <textarea rows="3" type="text" name="description" id="description" value="{{ old('description') }}" class="form-control"
                                aria-describedby="emailHelp" placeholder="Max 500 Charecters"></textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                                            ...
                                            <table class="table">
                                                <thead class="text-center">
                                                    <tr>
                                                        ...
                                                        <th class="fs-3 text-dark">Create</th>
                                                        <th class="fs-3 text-dark">Edit</th>
                                                        <th class="fs-3 text-dark">Approve</th>
                                                        <th class="fs-3 text-dark">Pay</th>
                                                    </tr>
                                                </thead>
                                                <tbody class="text-center text-dark">
                                                    @foreach ($modules as $module => $actions)
                                                        <tr>
                                                            <td class="text-start text-dark">{{ ucfirst($module) }}</td>
                                                            @foreach (['manage', 'view', 'create', 'edit', 'approve', 'pay'] as $action)
                                                                <td @if ($action === 'manage') class="bg-info-subtle" @endif>
                                                                    @php
                                                                        $permissionName = strtolower($mainModule . ':' . $action . ' ' . $module);
                                                                        $hideDeleteFor = ['payroll run']; // Add more module names to this array if needed
                                                                        $shouldShow = !(strtolower($action) === 'delete' && in_array(strtolower($module), $hideDeleteFor));
                                                                    @endphp
                                                                    @if ($shouldShow)
                                                                        <input
